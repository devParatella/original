...
const ReservationForm = () => {
  ...
  const handleAddProduct = () => {
    // Verifica se a acomodação está preenchido
    if (!selectedProduct) {
      Swal.fire({
        icon: 'warning',
        title: 'Atenção!',
        text: "Selecione pelo menos uma acomodação antes de continuar a reserva.",
      });
      return;
    }
    ...
    if (!duration) {
      Swal.fire({
        icon: 'warning',
        title: 'Atenção!',
        text: "Selecione a duração de tempo em diárias antes de continuar a reserva.",
      });
      return;
    }

    // Verifica se a data de checkin está preenchida
    if (!checkin) {
      Swal.fire({
        icon: 'warning',
        title: 'Atenção!',
        text: "Selecione uma data de checkin antes de continuar a reserva.",
      });
      return;
    }

    // Verifica se a data de checkout está preenchida
    if (!checkout) {
      Swal.fire({
        icon: 'warning',
        title: 'Atenção!',
        text: "Selecione uma data de checkout antes de continuar a reserva.",
      });
      return;
    }
  ...
  const gerarPDF = (reservation) => {
    ...
    function formatDate(dateString) {
      ...
      checkin.setHours(checkin.getHours() + 3);
          ...
          table: {
            ...
            body: [
              [
                ...
                { text: "checkin", style: "headerTable" },
                { text: "checkout", style: "headerTable" },
                { text: "Duração", style: "headerTable" },
                { text: "Status", style: "headerTable" },
                { text: "Repetir", style: "headerTable" },
              ],
          [
            { text: reservation?.id || "-", style: "item" }, // Tratamento para valor indefinido
            ...
            { text: reservation?.repeat || "-", style: "item" },
              ...
              [
                { text: "Valor Total", style: "headerTable", colSpan: 5 }, // ColSpan correto
                {}, {}, {}, {}, // Acomodação preenchida para manter o layout correto
